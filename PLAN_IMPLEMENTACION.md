...
- [~] `F06-T04` Implementar `elements_query` con atributos, estilo computable, parent/layer y bounds; remapear IDs para `--query-id`/`--query-all` y probar IDs con coma, punto y coma, whitespace, Unicode y controles rechazados. â€” `includeBounds=true` consulta `--query-all` sobre un bundle nativo inmutable con revisión obligatoria; `includeComputedStyle=true` resuelve presentación, CSS compuesto seguro, inline, herencia y `!important`, declarando `exact-supported` o limitaciones parciales. Faltan CSS universal, remapeo integral de IDs y corpus completo de IDs inseguros.
