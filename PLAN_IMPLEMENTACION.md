...
- [~] `F08-T31` Descubrir exporters INX desde directorios de datos, parsear sus manifests de forma segura y validarlos con export real; no existe un `--list-output-types` simétrico fiable. — Se descubren INX sólo desde directorios internos, con límite, XML sin DTD/entidades y sin exponer paths/IDs como inputs; el doctor informa únicamente el conteo. DXF y HPGL tienen smoke por adapter; los demás permanecen no anunciados hasta su propia prueba.
