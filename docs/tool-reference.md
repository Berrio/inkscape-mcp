...
| `palette_inspect`          | Colores hex directos, variables CSS locales y número de usos.    | Inventariar colores de un documento.                |
| `palette_apply`            | Mapa explícito `from`/`to` para colores y variables CSS locales. | Sustituir una paleta sin preferencias globales.     |
| `color_management_inspect` | Perfiles SVG locales y usos `icc-color()`.                       | Detectar límites CMYK sin convertir el documento.   |
...
| Tool                          | Schema/operación                                                    | Ejemplo de uso                                                                      |
| ----------------------------- | ------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| `document_export_preset_plan` | Documento/revisión, preset y directorio relativo.                   | Hacer preflight de `print-pdf-300dpi`; conservar token sólo para el siguiente paso. |
