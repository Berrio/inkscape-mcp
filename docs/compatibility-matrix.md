...
| EPS y PostScript                               | `document_import_postscript`              | Probado y gateado | Sonda controlada de conversión headless por instalación; exige firma `%!`, staging, revisión y manifest. La fidelidad nativa no está garantizada.                                        |
| AI/EMF/WMF/XAML/DXF y otros tipos del doctor   | Ninguno                                   | Bloqueado         | Que aparezcan como input type nativo no autoriza importarlos desde MCP; `document_import_capabilities` los mantiene bloqueados sin fixture headless.                                     |
