...
- [x] `F08-T07` Gatear AI/EPS/PS/EMF/WMF/XAML/DXF y otros según sonda real. — `document_import_capabilities` parte de `--list-input-types`, mantiene AI/EMF/WMF/XAML/DXF y otros como `detected-but-blocked`, y habilita EPS/PS sólo tras una sonda controlada de conversión headless. `document_import_postscript` no acepta formatos ni argumentos nativos libres, requiere firma y publica SVG saneado + manifest tras staging/revisión.
...
- [~] `F08-T09` Probar archivo corrupto, password/encrypted, huge pages, zip bomb y dependencia ausente. — SVGZ limita expansión, raster rechaza cabeceras truncadas/corruptas, PostScript rechaza firma corrupta antes de ejecutar/publicar y `document_import_pdf` rechaza `/Encrypt`, páginas ausentes y páginas por encima de 200in/50.000.000pt² antes de invocar Inkscape o publicar outputs. Quedan las dependencias de fuentes/perfiles.
