...
- [~] `F07-T36` Investigar LPE; publicar únicamente subconjunto headless con smoke tests. — `path_effects_inspect` expone efectos locales preservados y sus referencias; `path_effects_manage` desadjunta paths o borra efectos locales sin uso con revisión/backup. La edición de parámetros y smoke nativo por tipo de LPE siguen pendientes.

#### Cobertura avanzada adicional

- [~] `F07-T37` Implementar edición de nodos tipada: insertar/mover/eliminar, cambiar comando, handles, abrir/cerrar subpath y mantener índices/revisión. — `path_node_move` mueve endpoints M/L/H/V/T/Q/C/S/A y normaliza comandos relativos a SVG absoluto equivalente; `path_node_edit` inserta/elimina/retaggea L/T, abre/cierra subpaths, edita handles Q/C y parámetros A, y expande S/T a C/Q equivalentes antes de editar handles. Falta estabilidad de índices tras transformaciones complejas.
...
- [~] `F07-T39` Modelar conectores/diagramas como paths + markers + referencias, preservando semántica Inkscape cuando exista. — `connector_create` publica una polilínea con `inkscape:connector-type`, `connector_retarget` cambia endpoints locales y `connector_route` calcula una ruta ortogonal por centros para rect/circle/ellipse sin transforms. Evitación de obstáculos y follow-after-move siguen pendientes.
- [~] `F07-T40` Implementar inventario/aplicación de paletas y swatches documentales sin depender de preferencias globales. — `palette_inspect` inventaría colores hex locales, variables CSS y swatches sólidos nombrados de Inkscape; `palette_apply` reemplaza un mapa directo `from`/`to` con revisión y backup. Colores indirectos complejos y gradientes multistop siguen pendientes.
...
- [~] `F08-T09` Probar archivo corrupto, password/encrypted, huge pages, zip bomb y dependencia ausente. — SVGZ limita expansión, raster rechaza cabeceras truncadas/corruptas y `document_import_pdf` rechaza `/Encrypt` antes de invocar Inkscape o publicar outputs. Quedan fixture/política específicos para páginas PDF gigantes y dependencias de fuentes/perfiles.
