...
- [~] `F07-T39` Modelar conectores/diagramas como paths + markers + referencias, preservando semántica Inkscape cuando exista. — `connector_create` publica una polilínea con `inkscape:connector-type` y `connector_retarget` cambia endpoints locales sin alterar la ruta actual; routing automático y reconexión tras mover objetos permanecen pendientes.
