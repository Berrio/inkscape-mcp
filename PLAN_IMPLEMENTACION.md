...
- [x] `F10-T04` Implementar bearer auth obligatorio siempre que HTTP esté activo, con redacción/rotación local; no ofrecer modo anónimo ni siquiera en loopback. — `INKSCAPE_MCP_HTTP_TOKEN` conserva el token base64url único y `INKSCAPE_MCP_HTTP_TOKENS_FILE` permite hasta 32 principals desde un JSON interno releído por request; un reemplazo atómico rota sin reinicio y falla cerrado con 503 si la fuente es inválida. Las comparaciones usan digest de longitud fija/tiempo constante; la revisión formal vive en `docs/http-security.md`.
- [x] `F10-T05` Asociar documents/jobs/resources a autorización explícita, no a sesión MCP. — La factoría HTTP recibe `authInfo` del SDK y deriva un owner hash por principal+workspace para artifacts, snapshots, planes, jobs, recursos de documento y manifests. Las URIs resource se restringen al principal autenticado; stdio conserva el contrato anterior owner-bound por workspace.
- [x] `F10-T06` Añadir OpenTelemetry/logger HTTP estructurado. — Un `BasicTracerProvider` de OpenTelemetry exporta sólo `http_request_completed`, status, duración y correlación hash de principal a stderr; listener/rechazos conservan eventos mínimos. No hay etiquetas de URL, headers, token, cuerpo, rutas ni contenido documental.
- [-] `F10-T07` Ejecutar Inspector y conformance suite HTTP fijados en lockfile mediante scripts npm, con requirements/protocolo `2026-07-28` explícitos. — La sonda `npm run test:f10-inspector` confirma que Inspector 2.3.0 y Conformance 0.1.16 fijados sólo enumeran/hablan `2025-11-25`; el endpoint moderno responde `Unsupported protocol version` antes de despachar. No se presenta ese resultado como conformance moderna ni se anuncia HTTP estable; la sonda fallará para forzar reevaluación cuando ambas tools incorporen `2026-07-28`.
