...
- [ ] `F02-T22` Implementar `ArtifactStore` opaco sobre archivos autorizados/staging con metadata, ownership, TTL, `maxArtifactBytes` y límites separados; no devolver paths absolutos.
...
- [ ] `F03-T00` Cerrar ADR de `viewBoxPolicy`, escala/origen, coordinate spaces, IDs sintéticos/persistentes, motor/subconjunto CSS, fidelidad `exact|partial|approximate` y vectores normativos de la sección 10 antes de implementar resize.
- [ ] `F03-T01` Implementar `PhysicalLength`, `CssPixelLength`, `PageSize`, `UserPoint/UserRect`, matrices entre espacios y conversiones de viewport a 96 dpi.
- [ ] `F03-T02` Definir precisión/redondeo y pruebas de ida/vuelta.
- [ ] `F03-T03` Separar viewport físico, `viewBox`, user units y bounds del contenido.
...
- [ ] `F03-T07` Implementar SVG mínimo válido con namespaces Inkscape opcionales.
...
- [ ] `F06-T03` Implementar orden documental determinista y paginación.
...
- [ ] `F06-T05` Diferenciar elemento inexistente de selector vacío válido.

#### Creación

- [ ] `F06-T06` Crear rectángulo y rounded rect.
- [ ] `F06-T07` Crear círculo/elipse.
- [ ] `F06-T08` Crear line/polyline/polygon.
...
- [ ] `F06-T15` Implementar `elements_create` con batch acotado.
...
- [ ] `F06-T18` Implementar delete explícito y cascade/ref policy.
...
- [ ] `F06-T22` Registrar tools update/delete/duplicate/group.
...
- [ ] `F06-T36` Definir unión discriminada versionada `DesignOperation`.
- [ ] `F06-T37` Implementar `document_apply_operations` all-or-nothing.
- [ ] `F06-T38` Permitir referencias a IDs creados antes dentro de la misma transacción mediante aliases locales.
- [ ] `F06-T39` Implementar `dryRun` que valida y resume sin escribir.
- [ ] `F06-T40` Limitar operaciones, matches y costo estimado.
...
- [ ] `F07-T14` Crear/aplicar/liberar clip paths y masks.
...
- [ ] `F11-T01` Ejecutar fuzz/property tests de units, paths, selectors, XML, path data y export schemas.
