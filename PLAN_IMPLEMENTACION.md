...
- [~] `F08-T31` Descubrir exporters INX desde directorios de datos, parsear sus manifests de forma segura y validarlos con export real; no existe un `--list-output-types` simétrico fiable. — Se descubren INX sólo desde directorios internos, con máximo de archivos y lectura previa acotada en bytes; XML malformado, DTD/entidades e IDs inválidos se descartan sin exponer paths/IDs como inputs. El doctor informa únicamente el conteo. DXF, HPGL y FXG tienen smoke por adapter; la sonda GPL, igual que varias raster, no produce un artefacto verificable; los demás permanecen no anunciados hasta su propia prueba.
- [x] `F08-T32` Definir adapter versionado para outputs de extensión y evaluar DXF, HPGL/PLT, G-code y raster adicionales solo si están instalados y son headless. — DXF, HPGL y FXG quedan disponibles sólo por adapters fijos `inkscape-dxf/v1`/`inkscape-hpgl/v1`/`inkscape-fxg/v1`, con smoke real y verificación estructural. La instalación no declara PLT; G-code no declara output y pide directorio/postprocesador configurables; GPL, JPEG, TIFF y WebP instalados no produjeron artefacto headless verificable. Todos permanecen explícitamente ausentes del catálogo público.
- [x] `F08-T33` Rechazar parámetros/IDs arbitrarios de extensión; cada adapter publicado tiene schema, allowlist, threat model y fixture. — Los únicos adapters publicados son `inkscape-dxf/v1`, `inkscape-hpgl/v1` e `inkscape-fxg/v1`: formatos y `fidelityPolicy` literales, sin IDs, parámetros ni argv de extensión aportados por cliente; tienen pruebas unitarias, smoke MCP y limitaciones documentadas.
