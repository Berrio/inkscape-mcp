...
- [~] `F08-T32` Definir adapter versionado para outputs de extensión y evaluar DXF, HPGL/PLT, G-code y raster adicionales solo si están instalados y son headless. — DXF y HPGL quedan disponibles sólo por adapters fijos `inkscape-dxf/v1`/`inkscape-hpgl/v1`, con smoke real y verificación ASCII estructural. WebP instalado falló la sonda al no generar artefacto; G-code no declara output y pide directorio/postprocesador configurables. PLT y raster adicionales requieren la misma evaluación por capability.
