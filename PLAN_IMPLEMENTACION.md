...
- [~] `F07-T40` Implementar inventario/aplicación de paletas y swatches documentales sin depender de preferencias globales. — `palette_inspect` inventaría colores hex locales y variables CSS, y `palette_apply` reemplaza un mapa directo `from`/`to` con revisión y backup; colores indirectos complejos y swatches nombrados de Inkscape siguen pendientes.
