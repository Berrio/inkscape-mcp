...
Add-Content -LiteralPath $log -Value "[$([DateTime]::UtcNow.ToString('o'))] inkscape-mcp queue worker start ($mode)"
$output = & $node @arguments 2>&1
$exitCode = $LASTEXITCODE
$output | Tee-Object -FilePath $log -Append
Add-Content -LiteralPath $log -Value "[$([DateTime]::UtcNow.ToString('o'))] inkscape-mcp queue worker exit=$exitCode"
